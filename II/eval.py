...
def evaluate(model, dataloader, epoch, writer, logger, data_name='val', pb=False):
    # 每个模型的evaluate方式不同
    loss = model.evaluate(dataloader, epoch, writer, logger, data_name, pb)
    return loss
...
if __name__ == '__main__':
    ...
    if not proposal_box:
        ...
        evaluate(model, val_dataloader, which_epoch, writer, logger, 'val', pb=True)
