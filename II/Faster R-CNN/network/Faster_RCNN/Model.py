...
class Model(BaseModel):
    ...
    def forward_test_pb(self, image):  # test
        """给定一个batch的图像, 输出预测的[bounding boxes], 仅在验证和测试时使用"""
        image = list(im.to(opt.device) for im in image)
    ...
    def evaluate(self, dataloader, epoch, writer, logger, data_name='val', pb=False):
        if pb:
            return self.eval_pb(dataloader, epoch, writer, logger, data_name)
        else:
